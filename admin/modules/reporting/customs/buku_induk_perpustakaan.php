...
$page_title = 'BUKU INDUK PERPUSTAKAAN';
$reportView = false;
$num_recs_show = 10;
...
?>
    ...
    <fieldset>
    <div class="per_title">
    	<h2><?php echo __('Buku Induk Perpustakaan'); ?></h2>
	  </div>
    <div class="infoBox">
    ...
    <div id="filterForm">
        ...
        <div class="divRow">
            ...
            <div class="divRowContent"><input type="text" name="recsEachPage" size="3" maxlength="3" value="<?php echo $num_recs_show; ?>" /> <?php echo __('Set between 10 and 200'); ?></div>
        </div>
    </div>
...
<?php
	...
	if (isset($_GET['publish_year']) AND !empty($_GET['publish_year'])) {
        $publish_year = $dbs->escape_string($_GET['publish_year']);
	...
	if (isset($_GET['source']) AND !empty($_GET['source'])) {
        $source_options = $dbs->escape_string(trim($_GET['source']));
        $criteria .= ' AND i.source=\''.$source_options.'\'';
    }
    ...
    $subquery_str = '(SELECT DISTINCT bsub.biblio_id, bsub.gmd_id, bsub.title, bsub.isbn_issn, bsub.call_number, bsub.classification, bsub.language_id,
		bsub.publish_place_id, bsub.publish_year, bsub.publisher_id, bsub.collation, bsub.edition, bsub.input_date, bsub.sor
        FROM biblio AS bsub
    ...
    {
        // author name query
        $_biblio_q = $obj_db->query('SELECT b.title FROM biblio AS b
	...
	$xlsquery = 'SELECT b.biblio_id, b.input_date, b.title AS \''.__('Title').'\''.
		...
		$_SESSION['tblout'] = "buku_induk_perpustakaan";
