...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
  ...
  // e-DDC by Eko Wahyudi
  $str_input = '<div class="'.$visibility.'" ><a class="notAJAX button btn btn-danger openPopUp",scrolling="no", href="'.MWB.'bibliography/pop_ddc.php?biblioID='.$rec_d['biblio_id'].'" title="'.__('e-DDC for SLiMS &copy 2015').'" height="500" width="850">'.__('e-DDC').'</a> Search Classification Number with Quick e-DDC</div>';
  ...
  if ($sysconf['scanner'] !== false) {
    $str_input .= '<p>'.__('or scan a cover').'';
    ...
    if ($sysconf['scanner'] == 'html5') {
        ...
        $str_input .= '<p style="padding: 3px 0;"><label>'.__('History index:').' <input type="text" id="scan_history" value="1" style="width: 10px;" /></label> <input type="button" id="btn_getrecall" onclick="scan_recall" value="'.__('Recall').'" />';
        $str_input .= '<p style="padding: 3px 0;"><label>'.__('Host:').' <input type="text" id="scan_host" value="localhost" /></label> | <label>Port: <input type="text" id="scan_port" patter="\d*" maxlength="6" size="6" style="width: 60px;" value="8811" /></label> <input type="button" id="btn_getmachine" onclick="scan_init()" value="'.__('Get machine').'" />';
        $str_input .= '<p style="padding: 3px 0;"><label>'.__('Scanner:').' <select id="scan_machine" readonly><option>'.__('Default').'</option></select></label>';
        $str_input .= '<p style="padding: 3px 0;">'.__('Resolution').', <label>'.__('Horizontal:').' <input type="text" id="scan_res_x" value="300" pattern="\d*" maxlength="3" size="3" style="width: 30px;" />dpi</label> - <label>'.__('Vertical:').' <input type="text" id="scan_res_y" value="300" pattern="\d*" maxlength="3" size="3" style="width: 30px;" />dpi</label>';
        $str_input .= '<p style="padding: 3px 0;">'.__('Capture').', <label>'.__('Width:').' <input type="text" id="scan_capture_w" value="2550" pattern="\d*" maxlength="4" size="4" style="width: 30px;" />px</label> - <label>'.__('Height:').' <input type="text" id="scan_capture_h" value="3507" pattern="\d*" maxlength="4" size="4" style="width: 40px;" />px</label>';
        $str_input .= '<p>'.__('Result').', <label>'.__('Max Width:').' <input type="text" id="scan_max_w" value="360" pattern="\d*" maxlength="3" size="3" style="width: 30px;" />px</label> - <label>'.__('Max Height:').' <input type="text" id="scan_max_h" value="480" pattern="\d*" maxlength="3" size="3" style="width: 30px;" />px</label></div>';
        $str_input .= '<div id="scan_container" style=""><div style="height: 550px; width: 390px; overflow: auto; float: left;"><p>'.__('Scan result').'<img id="my_imgdata" style="margin: auto;" /></div>';
        $str_input .= '<div style="padding-left: 10px; height: 550; width: 400px; overflow: auto; float: left;"><p>'.__('Preview').' <input type="button" value="'.__('Rotate Left').'" onclick="scan_rotate(\'left\')" /> <input type="button" value="'.__('Rotate Right').'" onclick="scan_rotate(\'right\')" /><canvas id="my_selected" style="border: 1px solid #CCC; margin: auto;"></canvas></div></div></div></div>';
    }
  }
